...
<div class="card-form">
  ...
  <form method="post" action="guest-requests">
    ...
    <input type="hidden" name="id" value="${requestObj.requestId}"/>

    <p class="mb-3 text-muted">ID Yêu cầu: <strong>${requestObj.requestId}</strong></p>

    <div class="mb-3">
      <label class="form-label"><i class="fas fa-user me-2 text-primary"></i> Họ tên</label>
      <input type="text" name="fullName" class="form-control" value="${requestObj.fullName}" required/>
    </div>

    <div class="mb-3">
      <label class="form-label"><i class="fas fa-phone me-2 text-primary"></i> Điện thoại</label>
      <input type="text" name="phone" class="form-control" value="${requestObj.phone}" required/>
    </div>

    <div class="mb-3">
      <label class="form-label"><i class="fas fa-envelope me-2 text-primary"></i> Email</label>
      <input type="email" name="email" class="form-control" value="${requestObj.email}"/>
    </div>

    <div class="mb-3">
      <label class="form-label"><i class="fas fa-id-card me-2 text-primary"></i> CCCD</label>
      <input type="text" name="cccd" class="form-control" value="${requestObj.cccd}"/>
    </div>

    <div class="mb-3">
      <label class="form-label"><i class="fas fa-map-marker-alt me-2 text-primary"></i> Địa chỉ</label>
      <input type="text" name="address" class="form-control" value="${requestObj.address}"/>
    </div>

    <div class="mb-4">
      <label class="form-label"><i class="fas fa-comment-dots me-2 text-primary"></i> Lời nhắn</label>
      <textarea name="message" class="form-control">${requestObj.message}</textarea>
    </div>
    ...
    <div class="d-flex justify-content-between gap-2">
      ...
      <a href="guest-requests" class="btn btn-secondary w-50">
